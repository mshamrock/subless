/**
 * Production seed: verifiable facts only.
 *
 * Everything here is checkable — real categories, real paid services with their
 * real prices, and real open-source projects with real repositories. There are
 * no invented people, no invented votes, and above all no invented savings: the
 * North Star is the one number the whole brand rests on, and seeding it with
 * fiction would make the site's central claim a lie on day one.
 *
 *   npm run db:seed:prod                 services + catalogued alternatives
 *   npm run db:seed:prod -- --targets-only   services only, empty catalog
 *
 * The second form is closer to the brand brief §19, which argues for opening
 * with one question rather than a furnished directory. Use it if you would
 * rather the catalog be filled entirely by what the challenges produce.
 */
...
async function main() {
  await assertDatabaseFree("db:seed:prod");
  ...
  const { ensureTargetIcon } = await import("../src/lib/targets");
  const { slugify } = await import("../src/lib/utils");
  const { syncProject } = await import("../src/lib/sync");

  console.log(`Seeding ${targetsOnly ? "services only" : "services and catalogued alternatives"}\n`);

  // Idempotent throughout: this runs against a live database, and a second run
  // must not duplicate rows or overwrite anything the community has since added
  const categoryIds = new Map<string, number>();
  for (const c of CATEGORIES) {
    const [existing] = await db.select().from(s.categories).where(eq(s.categories.slug, c.slug)).limit(1);
    if (existing) {
  ...
  for (const t of TARGETS) {
    const [existing] = await db.select().from(s.targets).where(eq(s.targets.slug, t.slug)).limit(1);
    if (existing) {
      targetIds.set(t.slug, existing.id);
      if (!existing.logoData) await ensureTargetIcon(existing.id, existing.slug, existing.websiteUrl);
      continue;
    }
    const [row] = await db
      .insert(s.targets)
  ...
  console.log(`Services: ${targetIds.size}`);

  if (targetsOnly) {
    console.log("\nDone. The catalog starts empty and fills from the challenges.");
    process.exit(0);
  }
  ...
  for (const p of PROJECTS) {
    ...
    const [existing] = await db.select().from(s.projects).where(eq(s.projects.repoUrl, repoUrl)).limit(1);
    ...
    const synced = await syncProject(row.id);
    console.log(`  + ${p.name}${synced.ok ? "" : `  (metrics: ${synced.error})`}`);
    added++;
  }

  console.log(`\nCatalogued alternatives added: ${added}`);
